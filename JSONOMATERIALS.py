...
def fetch_jit_components():
    ...
    # Prepare the response with only materials
    response = []
    for i, component in enumerate(jitcalcomponents):
        materials = [item['Material'] for item in results[i] if item]  # Extract material numbers
        response.append({'component': component['MATERIAL'], 'materials': materials})
...
def fetch_jit_components_api():
    ...
    for i, component in enumerate(jitcalcomponents):
        ...
        materials = [item['Material'] for item in results[i] if item]  # Extract material numbers
        response.append({'CUST_MAT': cust_mat, 'materials': materials})
